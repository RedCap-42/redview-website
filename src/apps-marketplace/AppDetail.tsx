        ...
        <HeroRow>
          ...
          <HeroText>
            ...
            <HeroActions>
              <Button
                href={installUrl}
                label={i18n._(msg`Activer le module`)}
                variant="filled"
              />
              <VettedBadge>{i18n._(msg`Module officiel RedView`)}</VettedBadge>
            </HeroActions>
          </HeroText>
        </HeroRow>
      </SectionShell>
      ...
      <SectionShell rhythm="section" scheme="light">
        <ContentGrid>
              ...
              <ScreenshotList>
                {app.screenshots.map((screenshot) => (
                  <Screenshot
                    key={screenshot}
                    src={screenshot}
                    alt={i18n._(msg`Capture du module ${app.name}`)}
                    loading="lazy"
                  />
                ))}
              </ScreenshotList>
          ...
          <Sidebar>
            <Button
              href={installUrl}
              label={i18n._(msg`Explorer sur RedView`)}
              variant="filled"
            />
            <Divider aria-hidden="true" />
            <DetailRow>
              <DetailLabel>{i18n._(msg`Auteur / Source`)}</DetailLabel>
              <DetailValue>{app.author}</DetailValue>
            </DetailRow>
            {app.category.length > 0 && (
              <DetailRow>
                <DetailLabel>{i18n._(msg`Catégorie`)}</DetailLabel>
